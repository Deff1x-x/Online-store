...
[ApiController]
public sealed class PaymentsController(PaymentService service, ICurrentUser user, IHostEnvironment environment) : ControllerBase
{
    ...
    public async Task<IActionResult> Kaspi(JsonObject body,CancellationToken ct)
    {
        if (environment.IsProduction()) return StatusCode(503, new { message = "Kaspi webhook is disabled until a provider contract is configured", code = "kaspi_webhook_disabled" });
        return Ok(await service.HandleKaspiAsync(body,ct));
    }
}
